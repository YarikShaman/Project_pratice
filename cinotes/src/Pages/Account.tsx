...
export function Account() {
    ...
    function SetFavorite() {
        ...
        axios.post("https://back.cintoes.link/user-data/add",data, config);
    }
    ...
    useEffect(() => {
        if (CheckJWT() != 0)
            ...
            axios.get("https://back.cintoes.link/user-data/get?user_id=" + DecodeB64(localStorage["jwt"]).id, config)
                .then(res => {
                    setData(res.data)
                    if(res.data.FavFilm)
                    axios.get("https://back.cintoes.link/films/" + res.data.FavFilm.toString() + "/", config)
                        .then((resp) => {
                            setFilm(resp.data.title)
                        })
                    if(res.data.FavGenre)
                    axios.get("https://back.cintoes.link/films/genres/" + res.data.FavGenre.toString() + "/", config)
                        .then((resp) => {

                            setGenre(resp.data.title)
                        })
                    if(res.data.FavActor)
                    axios.get("https://back.cintoes.link/actors/" + res.data.FavActor.toString() + "/", config)
                        .then((resp) => {
                            setActor(resp.data.name)
                        })

                })
            axios.get("https://back.cintoes.link/films/?page_size=500", config)
                .then(res => {
            ...
            axios.get("https://back.cintoes.link/films/genres/?page_size=1000", config)
                .then((res) => {
