...
export function ReviewsInFilms({ comment, pk, onUpdateParentState }: { comment: any; pk: number; onUpdateParentState: () => void; }) {
    ...
    const like = () =>{
        axios.post(`https://back.cintoes.link/comment/like/`+comment.Id,{} ,config)
    }
    useEffect(() => {
        ...
        <div className={"flex m-4 bg-gray-600 rounded-2xl pb-2 flex-col"}>
            <div className={" flex flex-row"}>
                <div className={"rounded-2xl bg-black border-slate-950 w-[65px] border-4 m-[10px]"}>
                    <img className={"object-cover rounded-2xl h-[50px] w-[50px]"} src={comment.AvatarLink}/>
                </div>
            ...
            <div className={"flex flex-row"}>
                ...
                <div id={'Likes'} className={"flex flex-row justify-end content-end flex-wrap w-[200px]"}>
